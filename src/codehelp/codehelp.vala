...
namespace Vls.CodeHelp {
    ...
    Lsp.TextEdit format (Lsp.FormattingOptions options, Vala.SourceFile source) throws FormattingError {
        File config;
        try {
            config = CodeHelp.get_uncrustify_config (options);
        } catch (Error e) {
        ...
        try {
            ...
            uint8[] contents;
            FileUtils.get_data (source.filename, out contents);
            size_t bytes_written;
            subprocess.get_stdin_pipe ().write_all (contents, out bytes_written);
    ...
    File get_uncrustify_config (Lsp.FormattingOptions options) throws Error {
